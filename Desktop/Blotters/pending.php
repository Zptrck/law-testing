                ...
                include 'conn.php';
                    ...
                    if (!empty($search_query)) {
                        $sql = "SELECT * FROM blotter 
                                WHERE (`case_no` LIKE '%$search_query%' 
                                       OR `case_title` LIKE '%$search_query%' 
                                       OR `complainant_name` LIKE '%$search_query%' 
                                       OR `respondent` LIKE '%$search_query%' 
                                       OR `nature` LIKE '%$search_query%' 
                                       OR `date` LIKE '%$search_query%' 
                                       OR `time` LIKE '%$search_query%' 
                                       OR `dates` LIKE '%$search_query%') 
                                  AND `remarks` = 'Pending' 
                                ORDER BY `case_no` DESC";
                    }
                    else {
                        $sql="SELECT * FROM Blotter WHERE `remarks` = 'Pending' ORDER BY case_no DESC";
                    }
